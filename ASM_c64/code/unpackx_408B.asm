...
.const mmchl = 4
.var zpvars = $c0  //$D9 
.var BSS =  $d7    //$3DD // putting bss to ZP makes code 14 bytes less
.var BSSbuffer = $3DD 	  // but needs caching to not destroy basic
...
UNPACKX:   // unpacking pakerX packages
			...
			tax 				// we do not store dw (but mayne should) - 2 bytes
...
skipli:
			...
			ldy #3				// -0 byte, setupnv restores Y
...
packeddata:
//.import binary "spiral_N.pkx"
.import binary "spiral_back_N.pkx"
